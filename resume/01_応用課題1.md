...
まずは、 devise を用いた認証のみを行うアプリを作ってみましょう。
...
次にトップページを表示するためのコントローラとビューを用意しましょう。今回は、 *welcome/index* をトップページとします。
...
この状態でサーバを起動すると、 `http://localhost:3000/welcome/index` にアクセスすることで、 `welcome/index` が処理されて表示されます。

ここでは、このページをトップページにし、 `http://localhost:3000` にアクセスされた際に表示されるようにしたいので、`config/routes.rb` を下記のように変更します。
...
下記のコードでは `localhost` を指定していますが、

```patch
# config/environments/development.rb
  config.action_mailer.raise_delivery_errors = false

+  config.action_mailer.default_url_options = { host: '【workspace名】-【cloud9のUsername】' }
...
ログインしている時だけアクセスできるページ `/dashboard` を用意してください。
